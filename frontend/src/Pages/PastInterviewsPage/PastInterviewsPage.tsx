...
const PastInterviewsPage = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      setHistory(JSON.parse(saved) as InterviewSession[]);
    }
  }, []);
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <span
                  className={`px-3 py-1 rounded-full text-sm font-bold ${scoreBadgeClass(
                    session.avgScore
                  )}`}
                >
                  {session.avgScore}/10
            ...
            {/* Expanded: per-question feedback */}
                ...
                {session.feedback.map((f, j) => (
                  <div key={j} className="px-6 py-4 space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-bold ${scoreBadgeClass(
                          f.score
                        )}`}
                      >
                        {f.score}/10
